  ...
  const getSegmentStyle = (segment: DiffSegment, docType: 'ima' | 'assuta') => {
    ...
    if (segment.type === 'changed' && segment.diff_id) {
      ...
      if (selection === docType) {
        return `${baseStyle} bg-green-300 p-1 rounded`; // Selected version - more prominent green
      }
      // More distinct colors for unselected differences
      return `${baseStyle} ${docType === 'ima' ? 'bg-red-300' : 'bg-blue-300'} p-1 rounded`; 
    }
    return "inline"; // Common segments, no special styling beyond potential hover if needed
  };
